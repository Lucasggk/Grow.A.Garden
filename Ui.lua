	...
	function Element:New(Idx, Config)
			...
			then
				...
				Event = UserInputService.InputBegan:Connect(function(Input)
					...
					EndedEvent = UserInputService.InputEnded:Connect(function(Input)
						local match = false
						if Input.UserInputType == Enum.UserInputType.Keyboard and Input.KeyCode and Key and Input.KeyCode.Name == Key then
							match = true
		...
		Creator.AddSignal(UserInputService.InputEnded, function(Input)
			if Keybind.Mode == "Hold" then
				...
				elseif Input.UserInputType == Enum.UserInputType.Keyboard and Input.KeyCode and Key and Input.KeyCode.Name == Key then
					Keybind.Internal.Pressed = false
					if Keybind.Toggled then
						Keybind.Toggled = false
						Keybind:DoClick()
...
ElementsTable.Colorpicker = (function()
	...
	function Element:New(Idx, Config)
		...
		Creator.AddSignal(Preview.InputBegan, function(input)
		...
		Creator.AddSignal(UserInputService.InputBegan, function(input, processed)
			...
			if input.UserInputType == Enum.UserInputType.MouseButton1 and Popup.Visible then
				...
				if not (mousePos.X >= Popup.AbsolutePosition.X and mousePos.X <= Popup.AbsolutePosition.X + Popup.AbsoluteSize.X and
					mousePos.Y >= Popup.AbsolutePosition.Y and mousePos.Y <= Popup.AbsolutePosition.Y + Popup.AbsoluteSize.Y) then
					Popup.Visible = false
					Picking = false
				end
			end
		...
		Creator.AddSignal(HueBar.InputEnded, function(input)
			draggingHue = false
		end)

		Creator.AddSignal(SV.InputBegan, function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				draggingSV = true
			end
		end)
		Creator.AddSignal(SV.InputEnded, function(input)
			draggingSV = false
		end)
		...
		Creator.AddSignal(AlphaBar.InputEnded, function(input)
			draggingAlpha = false
		end)
...
ElementsTable.Input = (function()
	...
	function Element:New(Idx, Config)
		...
		Creator.AddSignal(Textbox.Input.Changed, function()
		...
		pcall(function()
			if isfile(self.Folder .. "/" .. name .. ".json") then
				...
				for k, v in pairs(data) do
					...
					if opt then
						if v.Type == "Toggle" and opt.SetValue then opt:SetValue(v.Value) end
						if v.Type == "Slider" and opt.SetValue then opt:SetValue(v.Value) end
						if v.Type == "Dropdown" and opt.SetValue then opt:SetValue(v.Value) end
						if v.Type == "Colorpicker" and opt.SetValue then opt:SetValue(v.Value, v.Transparency) end
						if v.Type == "Keybind" and opt.SetValue then opt:SetValue(v.Value, v.Mode) end
						if v.Type == "Input" and opt.SetValue then opt:SetValue(v.Value) end
					end
				end
			end
		end)
...
function Library:CreateWindow(Config)
	...
	if TabLogo and type(TabLogo) == "string" and TabLogo ~= "" then
		...
		pcall(function()
			if TabLogo ~= "" then Logo.Image = TabLogo end
		end)
	end
